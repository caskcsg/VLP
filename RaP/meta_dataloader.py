...
class MetaLoader(object):
    ...
    def __init__(self, loaders, accum_steps=1, distributed=False,epoch=None,output_dir=None):
        ...
        for n, l in loaders.items():
            ...
            l.sampler.set_epoch(epoch+1)
            self.name2loader[n] = l
            self.name2iter[n] = iter(l)
            self.sampling_pools.extend([n]*r)
        le =0 
        for key in self.name2loader.keys():
            le+=len(self.name2loader[key])
        self.le=le
    ...
    def __iter__(self):
        ...
        #如果跑完一次
        while len(self.sampling_pools)!=0:
            if self.step % self.accum_steps == 0:
                task = random.choice(self.sampling_pools)
                if self.distributed:
                    base_dir = self.output_dir
                    file_dir = os.path.join(base_dir,'task_temp_save.pkl')
                    if dist.get_rank()==0:
                        with open(file_dir,'wb+') as temp_file:
                            pickle.dump(task,temp_file)
                    dist.barrier()
                    with open(file_dir,'rb') as temp_file:
                        task = pickle.load(temp_file)
                    dist.barrier()
                    if dist.get_rank()==0:
            ...
            try:
                ...
                self.sampling_pools.remove(task)
                continue
            yield task, batch
    def __len__(self):
        """ this iterator will run indefinitely """

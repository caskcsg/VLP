...
class Rap_pretrain(nn.Module):
    def __init__(self,                 
        ...
        if vit=='base':
            ...
            # checkpoint=torch.load('https://dl.fbaipublicfiles.com/deit/deit_base_patch16_224-b5f2ef4d.pth',map_location='cpu')
            # state_dict = checkpoint["model"]
            # pos_embed_reshaped = interpolate_pos_embed(state_dict['pos_embed'], self.visual_encoder)
            # state_dict['pos_embed'] = pos_embed_reshaped
            # msg = self.visual_encoder.load_state_dict(state_dict,strict=False)
    ...
    def forward(self, image,image_aug, caption, alpha):
        ...
        image_aug = image_aug.reshape(image_aug.shape[0]*image_aug.shape[1],image_aug.shape[2],image_aug.shape[3],image_aug.shape[4])
...
#
        ...
        loss_t2i_crosMod_l = self.redundancy_weight(text_feat_m_l, image_feat, self.temp, text.attention_mask[:,1:],sim=sim_local_t2i_max)

        loss_i2t_crosMod_l = self.redundancy_weight(image_feat_m_l, text_feat, self.temp,sim = sim_local_i2t_max)
    ...
    def redundancy_weight(self, l, m, temp, attention_mask=None,sim=None):
